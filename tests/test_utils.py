...
def test_sorted_dict():
    test_dict = {
        "thirdKey": 3,
        "myDict": {"c": 2, "b": 2, "a": "one"},
        "firstKey": ["simple_value", {"twoValue": 2, "oneKey": "one"}],
        "zzzz": 2,
        "aaaa": 1,
    }
    expected_dict = OrderedDict(
        [
            ...
            (
                "firstKey",
                ["simple_value", OrderedDict([("oneKey", "one"), ("twoValue", 2)])],
            ),
...
def test_camelize_dict():
    ...
    expected_dict = {
        "thirdKey": 3,
        "myDict": {"Two_key": 2, "NGKeyword": 3, "oneKey": "one"},
        "myList": [{"Two_key": 2, "NGKeyword": 3, "oneKey": "one"}],
        "SecondKey": 2,
        "firstKey": 1,
    }
...
<request>
  <itemUpdateRequest>
    ...
    expected_no_pretty_xml_string = (
        expected_xml_string.replace("  ", "").replace("\n", "").replace("?>", "?>\n")
    )
    assert expected_no_pretty_xml_string == dict2xml(
        data, root="request", pretty_print=False
    )
