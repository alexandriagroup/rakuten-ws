...
def assert_response_is_valid(response, endpoint_method):
    ...
    valid_keys["IchibaItem/Ranking"] = ["lastBuildDate", "title", "Items"]
    valid_keys["IchibaGenre/Search"] = ["current", "parents", "children"]
    valid_keys["IchibaTag/Search"] = ["tagGroups"]
    valid_keys["IchibaProduct/Search"] = [
        "count",
        "hits",
        "last",
        "pageCount",
...
def test_item_ranking(ws, params):
    assert_response_is_valid(ws.ichiba.item.ranking(**params), "IchibaItem/Ranking")


@pytest.mark.parametrize("params", genre_search_data, ids=idfn)
def test_genre_search(ws, params):
    params.update(genreId=0)
    assert_response_is_valid(ws.ichiba.genre.search(**params), "IchibaGenre/Search")
...
def test_product_search(ws, params):
    params.update(keyword="Naruto")
    assert_response_is_valid(ws.ichiba.product.search(**params), "IchibaProduct/Search")
