...
def insert_category(ws, name):
    params = {"categorySetManageNumber": 0, "categoryId": 0, "category": {"name": name}}
...
def test_category_api(ws):
    ...
    result = ws.rms.category.move_category(
    ...
    result = ws.rms.category.get_categories(categorySetManageNumber=0)
    assert (
        result["categoryList"]["category"][-1]["childCategories"]["category"][
            "categoryId"
        ]
        == params["categoryId"]
    )
